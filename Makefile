LANGUAGE_NAME := tree-sitter-iss
HOMEPAGE_URL := https://github.com/yanuoma/tree-sitter-iss
VERSION := 0.1.0
DESCRIPTION := Inno Setup (.iss) grammar for tree-sitter
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/iss '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/iss
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/iss
